...
| Tag | Etichetta che raggruppa note in modo trasversale alle cartelle; è anche uno dei metadati della nota (RF-04, RF-06). I tag sono gerarchici e si possono annidare. Si usa sempre "tag", mai "hashtag" o "categoria" | Cartella |
| Metadati | Informazioni associate a una nota: titolo, date, tag (RF-04) | Contenuto della nota |
| Data di fine validità | Data, scelta dall'utente, oltre la quale la nota non è più considerata valida. Non ha effetti automatici (RF-04) | Data di ultima modifica |
| Blocco (lock) | Protezione con password di un workspace o di una nota (RF-09) | Cifratura end-to-end |
